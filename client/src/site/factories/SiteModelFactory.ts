...
export class SiteModelFactory {
  ...
  static build(id: number, numOfChildren: number) {
    ...
    for (let i = 0; i < numOfChildren; i++) {
      const squad = new SquadronModel(
        Number(id.toString() + i.toString()),
        `Squadron ${id.toString() + i.toString()}`,
        []
      );
      for (let j = 0; j < numOfChildren; j++) {
        const flight = new FlightModel(
          Number(id.toString() + i.toString() + j.toString()),
          `Flight ${id.toString() + i.toString() + j.toString()}`
        );
